...
export async function getAlltimeLanguagesCard(params) {
  ...
  const topLang = topLanguages[0]?.name || 'Unknown';
    headingText = `My top language is ${topLang}`;
  } else {
    ...
    switch (chart_type) {
      case 'bar': {
        const barBlocks = topLanguages.map((lang, i) => {
            ...
            blocks.push(
            `<text x="${x + barWidth / 2}" y="${chartBase + 12}" font-weight="bold" font-size="10" text-anchor="middle" fill="#${text_color}">${lang.name}</text>`
            );
      ...
      case 'bar_vertical': {
        ...
        const barBlocks = topLanguages.map((lang, i) => {
          ...
          blocks.push(`<text x="${leftPadding - 8}" y="${y + 9}" font-size="10" text-anchor="end" fill="#${text_color}">${lang.name}</text>`);
      ...
      case 'line': {
        ...
        const points = topLanguages.map((lang, i) => {
          ...
          return { x, y, seconds, label: lang.name, pct };
        });
        ...
        const labels = points.map((p, i) => {
          ...
          const timeLabel = !hide_time
            ...
            : '';
          const langLabel = `<text x="${p.x}" y="${chartBase + 12}" font-weight="bold" font-size="10" text-anchor="middle" fill="#${text_color}" fill-opacity="0.7">${label}</text>`;
      ...
      case 'radar': {
        ...
        const points = topLanguages.map((lang, i) => {
          ...
          return { x, y, angle, seconds, label: lang.name, pct };
        });
        ...
        const labels = topLanguages.map((lang, i) => {
          ...
          return `<text x="${x}" y="${y}" font-size="10" text-anchor="middle" alignment-baseline="middle" fill="#${text_color}">${lang.name}</text>`;
        });
