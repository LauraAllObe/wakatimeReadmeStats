...
export async function getWeekdayAverageCard({
  ...
  const candidateUrls = [
    `https://wakatime.com/api/v1/users/${username}/insights/weekdays/last_year`,
    `https://wakatime.com/api/v1/users/${username}/insights/weekdays?range=last_year`,
  ...
  for (const apiUrl of candidateUrls) {
    ...
    if (res.ok && json?.data?.weekdays?.length > 0) {
      weekdayAvg = json.data.weekdays;
      break;
    }
  ...
  if (!weekdayAvg || weekdayAvg.length === 0) {
    throw new Error(lastError || 'No weekday average insight data available.');
  }
