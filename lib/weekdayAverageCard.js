...
export async function getWeekdayAverageCard({
  ...
  const apiUrl = `https://wakatime.com/api/v1/users/current/insights/weekdays/last_year`;
  ...
  if (!res.ok || !json?.data?.weekdays || json.data.weekdays.length === 0) {
    throw new Error(json?.error || 'No weekday average insight data available.');
  }
