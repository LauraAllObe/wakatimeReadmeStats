...
export function svgContainer({
  ...
  const wrappedComponents = scaledComponents.map(({ content, height, originalWidth, finalScale, type, align }) => {
    const cleanedContent = content.replace(svgTagRegex, '').trim();
    const scaledWidth = originalWidth * finalScale;
    const shouldLeftAlign = align === 'left' || type === 'basic' || originalWidth <= 0;
    const offsetX = shouldLeftAlign
      ? 0
      : (svgEffectiveWidth - scaledWidth) / 2;
