...
export async function getProjectBreakdownCard(params) {
  ...
  const projectColors = {};
    ...
    do {
        newColor = mixed_colors ? varyColor(chart_color, 100) : chart_color;
        attempts++;
    } while (
        mixed_colors &&
  ...
  const chartSvgBlocks = (() => {
    switch (chart_type) {
      case 'bar': {
        ...
        const svgWidth = Math.max(chartWidth, 300) + 128;
        ...
        height = Math.max(height, chartBase + legendYOffset + legendHeight - 10);
      ...
      case 'area': {
        ...
        const projectPoints = {};
        const projectBaseY = Array(days.length).fill(chartBase); // Starting from bottom
        ...
        for (const project of allProjects) {
            ...
            const seconds = projectSeries[project][i];
            const height = maxSeconds ? (seconds / maxSeconds) * chartHeight : 0;
            const x = xPositions[i];
            const y = projectBaseY[i] - height;

            projectPoints[project].push({ x, y, seconds });
            projectBaseY[i] = y; // update base for next stacked layer
            }
        }
        ...
        for (const project of allProjects) {
            const points = projectPoints[project];
            const color = projectColors[project];
            ...
            const bottomPoints = points.map((p, i) => ({
            x: p.x,
            y: chartBase
            })).reverse();
            ...
            const areaPath = `
            ${topLine}
            ${bottomPoints.map((p, i) => `L ${p.x} ${p.y}`).join(' ')}
            Z
            `;
      ...
      case 'radar': {
        ...
        const legendColWidth = 12 + 10 + labelWidth + 10; // circle + spacing + label + padding
        const totalLegendWidth = legendCols * legendColWidth;
        const svgWidth2 = Math.max(chartWidth, 300) + 128;
