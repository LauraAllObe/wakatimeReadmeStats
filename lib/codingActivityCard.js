  ...
  const chartSvgBlocks = (() => {
    switch (chart_type) {
      ...
      case 'bubble': {
        ...
        const chartSize = Math.max(chartWidth, 240); // reduce layout canvas
        const packLayout = pack()
          .size([chartSize, chartSize * 0.75]) // make it vertically tighter
          .padding(2);

        const packedRoot = packLayout(root);
        const leaves = packedRoot.leaves();

        // Shift the entire bubble layout down
        const bubbleYOffset = 30; // adjust this value as needed
        leaves.forEach(node => {
          node.y += bubbleYOffset;
        });

        // Update height to fit packed layout
