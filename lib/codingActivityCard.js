    ...
    switch (chart_type) {
      case 'bar': {
        return days.map((d, i) => {
          ...
          const dayName = weekdayNames[new Date(d.range.date + 'T00:00:00Z').getUTCDay()];
      ...
      case 'line': {
        ...
        if (curved && points.length > 1) {
          pathD = catmullRom2bezier(points);
        } else {
        ...
        const labels = points.map((p, i) => {
          ...
          const dayName = weekdayNames[new Date(days[i].range.date + 'T00:00:00Z').getUTCDay()];
      ...
      case 'area': {
        ...
        let linePath = '';
        if (curved && points.length > 1) {
          linePath = catmullRom2bezier(points);
        } else {
      ...
      case 'bubble': {
        const referenceSeconds = 100800; // 27h 56m
        const scalingFactor = Math.sqrt(totalSeconds / referenceSeconds);
        const baseRadius = Math.min(50, Math.max(20, 35 * scalingFactor)); // clamp radius between 20–50
        const centerX = (chartWidth + 48) / 2;
        const centerYOffset = 110 * scalingFactor;
        const centerY = chartTop + (chartHeight + centerYOffset) / 2;
        const placed = [];
        const maxAttempts = 100;
        const legendYOffset = 130;
        ...
        function noOverlap(x, y, r) {
          return placed.every(p => {
            ...
            return dist > (r + p.r + 2); // Add padding
          });
        }
        ...
        const centerOffsetX = ((chartWidth + 100) - totalLegendWidth) / 2;

        days.forEach((d, i) => {
          const seconds = d.grand_total.total_seconds;
          const pct = ((seconds / totalSeconds) * 100).toFixed(1);
          const r = maxSeconds ? Math.max(6, (seconds / maxSeconds) * baseRadius) : 6;
          const color = varyColor(chart_color);
          const shortTime = formatShortTime(seconds);
          const dayName = weekdayNames[new Date(d.range.date + 'T00:00:00Z').getUTCDay()];

          // Try placing near center and spiral out if overlapping
          let angle = 0;
          let radius = 0;
          let cx, cy;
          let attempts = 0;
          do {
            ...
            radius += 0.8;
            attempts++;
          } while (!noOverlap(cx, cy, r) && attempts < maxAttempts);

          placed.push({ x: cx, y: cy, r });

          elements.push(`<circle cx="${cx}" cy="${cy}" r="${r}" fill="#${color}" fill-opacity="0.85" />`);
        ...
        days.forEach((d, i) => {
          ...
          elements.push(`
            <text font-size="9" fill="#${text_color}">
              <textPath href="#${dayPathId}" startOffset="50%" text-anchor="middle">${dayName}</textPath>
            </text>
          `);
      ...
      case 'spiral': {
        ...
        days.forEach((d, i) => {
          ...
          const dotColor = darkenColor(chart_color, 60);
          const lineColor = darkenColor(chart_color, 60);

          const shortTime = formatShortTime(seconds);
          const pct = (ratio * 100).toFixed(1) + '%';
          const day = weekdayNames[new Date(d.range.date + 'T00:00:00Z').getUTCDay()];
        ...
        const nowDotColor = darkenColor(chart_color, 60);

        elements.push(`<circle cx="${nowX}" cy="${nowY}" r="${nowRadius.toFixed(2)}" fill="#${nowDotColor}" />`);
        elements.push(`<line x1="${nowX}" y1="${nowY}" x2="${nowX}" y2="${nowLabelY}" stroke="#${darkenColor(chart_color, 60)}" stroke-width="1.2" />`);
        labels.push(`
