...
export async function getStarRankCard({
  ...
  try {
    ...
    const thresholdsPath = path.join(process.cwd(), 'cache', 'tierThresholds.json');
    try {
      const thresholdsRaw = fs.readFileSync(thresholdsPath, 'utf-8');
      tierThresholds = JSON.parse(thresholdsRaw);
    } catch (err) {
      console.error('⚠️ Failed to load tier thresholds from cache:', err.message);
      tierThresholds = [];
    }
  ...
  const rightContent = `
    ...
    <rect x="${rhsX}" y="65" width="${barWidth}" height="8" fill="#${text_color}" rx="4"/>
