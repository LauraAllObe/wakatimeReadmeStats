...
const GITHUB_TIER_THRESHOLDS = [
  ...
  { tier: 5, maxPercentile: 12.5 },
  { tier: 4, maxPercentile: 25 },
  { tier: 3, maxPercentile: 37.5 },
  { tier: 2, maxPercentile: 50 },
  { tier: 1, maxPercentile: 62.5 },
...
export async function getStarRankCard({
  ...
  let activeSource = (default_source ?? 'waka').toLowerCase() === 'waka' ? 'waka' : 'github';
  ...
  const progressText = isGithubSource
    ? (githubTierInfo?.next
      ? `Score ${level_value.toFixed(1)}/${level_target.toFixed(1)}`
