...
async function fetchGithubStats(username, githubToken) {
  ...
  return {
    ...
    repos: cc.repositoriesContributedTo?.totalCount ?? 0,
    stars,
...
export async function getStarRankCard({
  ...
  let activeSource = sourcePref === 'waka' ? 'waka' : (hasGithubToken ? 'github' : 'waka');
  ...
  const progressNeeds = {};
  if (isGithubSource && mode === 'progress' && activeSourceGithubStats && targetPercentile !== null) {
    const keys = ['commits', 'prs', 'issues', 'reviews', 'repos', 'stars', 'followers'];
    keys.forEach(key => {
      const base = activeSourceGithubStats[key] ?? 0;
      let low = 0;
      let high = Math.max(10, base * 2 + 10);
      const clone = { ...activeSourceGithubStats };
      const currentPercentile = computePercentile(clone);
      if (currentPercentile <= targetPercentile) {
        progressNeeds[key] = 0;
        return;
      }
      // expand high until reachable or cap
      const cap = 1_000_000;
      const reach = () => {
        clone[key] = base + high;
        return computePercentile(clone) <= targetPercentile;
      };
      while (!reach() && high < cap) {
        high *= 2;
      }
      // binary search
      low = 0;
      while (low < high) {
        const mid = Math.floor((low + high) / 2);
        clone[key] = base + mid;
        if (computePercentile(clone) <= targetPercentile) {
          high = mid;
        } else {
          low = mid + 1;
        }
      }
      progressNeeds[key] = low;
    });
  }
  ...
  let rightContent;
  if (isGithubSource && mode === 'stats' && activeSourceGithubStats) {
    const lines = [
      { label: 'Repos Contributed', value: formatNum(activeSourceGithubStats.repos) },
      { label: 'Commits', value: formatNum(activeSourceGithubStats.commits) },
      { label: 'PRs', value: formatNum(activeSourceGithubStats.prs) },
      { label: 'Issues', value: formatNum(activeSourceGithubStats.issues) },
      { label: 'Reviews', value: formatNum(activeSourceGithubStats.reviews) },
      { label: 'Stars', value: formatNum(activeSourceGithubStats.stars) },
      { label: 'Followers', value: formatNum(activeSourceGithubStats.followers) }
    ];
    const statsLines = lines.map((l, idx) =>
      `<text x="${rhsX}" y="${30 + idx * 15}" font-size="11" fill="#${text_color}" font-family="${font_family}">${l.label}: ${l.value}</text>`
    ).join('\n');
    rightContent = `
      <text x="${rhsX}" y="15" font-size="11" fill="#${text_color}" font-family="${font_family}" font-weight="bold">GitHub Stats</text>
      ${statsLines}
    `;
  } else if (isGithubSource && mode === 'progress' && activeSourceGithubStats) {
    const lines = [];
    if (targetPercentile === null) {
      lines.push('Top tier reached');
    } else {
      const labels = {
        repos: 'Repos',
        commits: 'Commits',
      ...
      Object.entries(labels).forEach(([key, label]) => {
        const need = progressNeeds[key];
        const msg = need > 0 ? `+${formatNum(need)} ${label}` : `${label}: good`;
        lines.push(msg);
      });
    }
    const progressLines = lines.map((t, idx) =>
      `<text x="${rhsX}" y="${30 + idx * 15}" font-size="11" fill="#${text_color}" font-family="${font_family}">${t}</text>`
    ).join('\n');
    rightContent = `
  ...
  const cardWidth = rhsX + barWidth + 40;
