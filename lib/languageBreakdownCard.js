...
function adjustTextColorForBackground(baseTextColor, fillColor, opacity) {
  // baseTextColor: string like "eeeeee"
  // fillColor: string like "#005ce3"
  // opacity: number from 0.2 to 0.9
...
function generateYAxisElements(maxSeconds, chartTop, chartBase, chartHeight, text_color, chartWidth, chart_type, y_axis_label, leftPadding) {
  ...
  const xTickStart = leftPadding + 4;     // Start of tick lines
  const xLabel = leftPadding - 8;         // Position of tick labels
  const xAxisEnd = chartWidth + 75;
  ...
  if (y_axis_label && chart_type !== 'radar') {
    const labelX = 28; // fixed position for vertical text
...
export async function getLanguageBreakdownCard(params) {
  const {
    username,
    text_color,
    chart_color,
  ...
  if (start_day && start_day !== '-7') {
    ...
    headingText = `This week, I mostly wrote ${topLanguage}`;
    } else {
      ...
      case 'bar': {
        ...
        days.forEach((d, i) => {
            ...
            dayBlocks.push(`
                ...
                <title>${lang}: ${formatShortTime(seconds)}</title>
                </rect>
            `);
            }
        ...
        const legendItems = !hide_legend ? allLanguages.map((lang, i) => {
            ...
            <text x="${x + 12}" y="${y}" font-size="10" fill="#${text_color}">${lang}</text>
            `;
        }) : [];
      ...
      case 'line': {
        ...
        for (const lang of allLanguages) {
            ...
            <circle cx="${p.x}" cy="${p.y}" r="2" fill="#${color}">
                <title>${lang}: ${formatShortTime(p.seconds)}</title>
            </circle>
            `));
    ...
    case 'radar': {
      ...
      const languagePolygons = allLanguages.map(lang => {
        ...
        return `
          ...
          ${points.map(p => `<circle cx="${p.x}" cy="${p.y}" r="2.2" fill="#${color}"><title>${lang}: ${formatShortTime(p.seconds)}</title></circle>`).join('\n')}
        `;
      });
      ...
      console.log("Radar legend color for first item:", allLanguages[0], languageColors[allLanguages[0]]);
      const legendItems = allLanguages.map((lang, i) => {
        ...
        const darkness = Math.round(5 + Math.pow(ratio, 1.8) * 20);
        const fill = rawColor;
        ratio = 0.5*(maxSeconds / (valueSeconds * 0.8));
        const fillOpacity = lightenOpacity(ratio);
        const adjustedTextColor = adjustTextColorForBackground(text_color, fill, fillOpacity);

        if (i === 0) {
          console.log('RAW', rawColor, 'BASE', baseColor, 'FILL', fill);
        }

        const pillText = `${formatShortTime(valueSeconds)} (${((valueSeconds / totalSeconds) * 100).toFixed(1)}%)`;

        return `
          <text x="${x}" y="${y}" font-size="10" font-weight="bold" fill="#${text_color}">${lang}</text>
          <rect x="${x + nameBoxWidth}" y="${y - 10}" width="${pillBoxWidth}" height="14" rx="4" ry="4" fill="#${fill}"  fill-opacity="${fillOpacity}" />
          <text x="${x + nameBoxWidth + pillBoxWidth / 2}" y="${y + 1}" font-size="9" text-anchor="middle" fill="#${adjustedTextColor}">${pillText}</text>
        `;
      });
