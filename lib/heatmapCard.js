export async function getHeatmapCard({
  ...
  const hasGithub = (default_source ?? 'github') === 'github' && github_token;
    ...
    try {
      ...
      weeks.forEach(week => {
        week.contributionDays?.forEach(day => {
          if (day.date) byDate[day.date] = day.contributionCount || 0;
        });
      });
    } catch (err) {
      console.error('GitHub heatmap fallback to WakaTime:', err.message);
    }
  }

  if (!hasGithub || Object.keys(byDate).length === 0) {
  ...
  const headingText =
    heading_type === 'friendly'
      ...
      : (hasGithub ? 'GITHUB ACTIVITY LAST YEAR' : 'ACTIVITY LAST YEAR');
