...
function getArrowSVG(diff, color = '#000', size = 14) {
  const upArrow = `
    <svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="${color}" stroke-width="4"
...
export async function getSpedometerCard({
  ...
  try {
    ...
    console.warn(`[Spedometer] Failed to fetch leaders data → fallback to avgSeconds.`, err.message);
  }
  ...
  const labelSets = {
    ...
    emoji: ['🐢', '🐥', '🐇', '🐎', '🦅'],
    emoji2: ['🌱', '🌿', '🌾', '🌳', '🌲'],
    emoji3: ['🔥', '💥', '⚡', '🌋', '☄️'],          // Fire-themed
    emoji4: ['🚶', '🚴', '🏎️', '🚀', '🛸'],          // Transportation-themed
    emoji5: ['📘', '📗', '📙', '📕', '🎓'],          // Learning-themed
    emoji6: ['✨', '🔮', '🧚‍♀️', '🌈', '🦄'],         // Magical-themed
    emoji7: ['🥉', '🥈', '🥇', '🏆', '🎖️'],          // Medal-themed
  ...
  const segments = [
    { start: 0, end: 36, color: chartColors[0], label: chosenLabels[0] },
    { start: 36, end: 72, color: chartColors[1], label: chosenLabels[1] },
    { start: 72, end: 108, color: chartColors[2], label: chosenLabels[2] },
    { start: 108, end: 144, color: chartColors[3], label: chosenLabels[3] },
    { start: 144, end: 180, color: chartColors[4], label: chosenLabels[4] }
  ];
