  ...
  `;
    ...
    if (highScorePercent <= 100) {
        ...
        <line x1="${inner.x}" y1="${inner.y}" x2="${outer.x}" y2="${outer.y}"
                stroke="${lighterHex}" stroke-width="2" stroke-dasharray="4 2" />
        <text x="${labelPos.x}" y="${labelPos.y}" font-size="10" font-weight="bold" fill="${lighterHex}"
                font-family="${font_family}" text-anchor="middle"
                transform="rotate(${highScoreAngle}, ${labelPos.x}, ${labelPos.y})">
            PB ⚑
        </text>
        `;
    }
