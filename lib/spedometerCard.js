...
export async function getSpedometerCard({
  ...
  const [todayData, yearlyStats] = await Promise.all([
    fetch(`https://wakatime.com/api/v1/users/${safeUsername}/durations?date=today`, { headers }).then(res => res.json()),
    fetch(`https://wakatime.com/api/v1/users/${safeUsername}/stats/last_year`, { headers }).then(res => res.json())
  ]);
