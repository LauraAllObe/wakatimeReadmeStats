    ...
    switch (chart_type) {
      case 'bar': {
        const barBlocks = topProjects.map((proj, i) => {
            ...
            blocks.push(
              `<text x="${x + barWidth / 2}" y="${chartBase + 10}" font-weight="bold" font-size="9" text-anchor="middle" fill="#${text_color}">${line1}</text>`
            );
            if (line2) {
              blocks.push(
                `<text x="${x + barWidth / 2}" y="${chartBase + 20}" font-weight="bold" font-size="9" text-anchor="middle" fill="#${text_color}">${line2}</text>`
              );
            }

            if (!hide_percentage) {
            blocks.push(
                `<text x="${x + barWidth / 2}" y="${chartBase + 34}" font-size="9" text-anchor="middle" fill="#${text_color}">${pct}%</text>`
            );
            }
      ...
      case 'bar_vertical': {
        ...
        const barBlocks = topProjects.map((proj, i) => {
          ...
          blocks.push(`<text x="${130 + leftPadding - 8}" y="${y + 9}" font-size="10" text-anchor="end" fill="#${text_color}">${truncateLabel(proj.name, 20)}</text>`);
          blocks.push(`<rect x="${130 + leftPadding}" y="${y}" width="${barWidth}" height="12" fill="#${fill}" rx="2" ry="2" />`);

          if (!hide_time && hide_percentage) {
            blocks.push(`<text x="${130 + leftPadding + barWidth + 6}" y="${y + 9}" font-size="9" fill="#${text_color}">${shortTime}</text>`);
          }

          if (!hide_time && !hide_percentage) {
            blocks.push(`<text x="${130 + leftPadding + barWidth + 6}" y="${y + 9}" font-size="9" fill="#${text_color}">${shortTime}    |    ${pct}%</text>`);
          }

          if (!hide_percentage && hide_time) {
            blocks.push(`<text x="${130 + leftPadding + barWidth + 6}" y="${y + 9}" font-size="9" fill="#${text_color}">${pct}%</text>`);
          }
      ...
      case 'radar': {
        ...
        const labels = topProjects.map((proj, i) => {
          ...
          return `<text x="${x}" y="${y}" font-size="10" text-anchor="middle" alignment-baseline="middle" fill="#${text_color}">${truncateLabel(proj.name, 6)}</text>`;
        });
        ...
        const legendItems = topProjects.map((proj, i) => {
          ...
          const useLighten = isDarkColor(bg_color);
          const adjustColor = useLighten ? lightenColor : darkenColor;
          const adjustedColor = adjustColor(chart_color, darkness);

          return `
            <text x="${x}" y="${y}" font-size="10" font-weight="bold" fill="#${text_color}">
              ${label}:
            </text>
            <rect x="${pillX}" y="${y - 10}" width="${uniformPillWidth}" height="14" fill="#${adjustedColor}" fill-opacity="0.2" />
        ...
        leaves.forEach((node, i) => {
          ...
          if (!hide_legend) {
            ...
            legendItems.push(`
              ...
              <text x="${x + legendTextOffset + 135}" y="${y + 3}" font-size="10" fill="#${text_color}">
                ${shortTime} (${pct}%)
              </text>
            `);
          }
